...
/// Opens the SurfaceType create/edit modal (mirrors the Country golden template;
/// Name is required, Description is optional and multi-line).
Future<void> showSurfaceTypeForm(
  ...
  return showDialog<void>(
    context: context,
    builder: (_) => _SurfaceTypeForm(existing: existing),
  );
}

class _SurfaceTypeForm extends ConsumerStatefulWidget {
  const _SurfaceTypeForm({this.existing});

  final SurfaceType? existing;

  @override
  ConsumerState<_SurfaceTypeForm> createState() => _SurfaceTypeFormState();
}

class _SurfaceTypeFormState extends ConsumerState<_SurfaceTypeForm> {
  ...
  Future<void> _submit() async {
    ...
    try {
      if (_isEdit) {
        await repo.updateSurfaceType(widget.existing!.id, payload);
      } else {
        await repo.createSurfaceType(payload);
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _isEdit
                ? 'Surface type "$name" updated.'
                : 'Surface type "$name" created.',
          ),
        ),
      );
  ...
  Widget build(BuildContext context) {
    return Dialog(
      child: Form(
        key: _formKey,
        child: FormScaffold(
          title: _isEdit ? 'Edit Surface Type' : 'New Surface Type',
